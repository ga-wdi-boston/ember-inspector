...
```md
That view states can be broken down using the "view tree", which aids with debuggin.
In addition, the ember inspector allows you to view specific details about an ember class (e.g.
by using the ember inspector($E) in the console).  Also, the ember route option within the
ember inspector is also beneficial in determining which classes interact with a specific root.  It
also provides information related to controllers.  Even class details are displayed in the console using the ($E) option.  Finally, the depreciations tab is beneficial in serving as a linter for
ember code.
```
...
```md
You would use the Data tab within the inspector.  After clicking on a model, one of the sidebar
options is to get attribute information.
```
