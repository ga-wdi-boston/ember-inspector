    ...
    */
    ...
    value: function loadFactories(factoryMap) {
      ...
      _keys(factoryMap).forEach(function (type) {
        var collectionName = _this2.schema ? pluralize(camelize(type)) : pluralize(type);
    ...
    value: function create(type, overrides, collectionFromCreateList) {
      ...
      if (this.schema && this.schema[pluralize(camelize(type))]) {
        var modelClass = this.schema[pluralize(camelize(type))];
    ...
    value: function createList(type, amount, overrides) {
      var list = [];
      var collectionName = this.schema ? pluralize(camelize(type)) : pluralize(type);
